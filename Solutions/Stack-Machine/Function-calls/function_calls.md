...
In this subcategory we learn about the function call and we will get to implement some simple functions later. A function is made out of three parts: the function call, the function content and the return. 
...
We push the address of the arguments, the address of the locals, and the address of the instruction right after the jump where the function is executed.
```
push.static ARGS
push.static LOCALS
push.value after
...
After that we proceed with jumping to the function block. Right after this jump we must have the label we previously pushed on the stack for the return address.
```
goto functionName
label after
```
...
Overall we managed to prepare the stack before executing the instruction in the function and then restore the program state before the call after the function finished executing.
...
## Push argument
This macro is used inside the function to push on stack values indexed from ARGS address. We first get access to argument we want to push by adding an index to the ARGS address, and then we push this value on the stack.
```
A = ARGS
D = *A
A = index
...
## Sub
A similar level, but instead of addition we implement subtractions of two arguments. In this case the order of the arguments matter. The first argument we push is the one we subtract from.
```
function sub 0
    push.argument 0
    push.argument 1
...
## putChar
In this level we implement another easy function. Using this function we can send inputs to a typewriter. We just have to get the argument value we want to be send, and store it on the typewriter input address.
```
function putChar 0
    push.argument 0
    pop.D
