...
The question is how do we get from reading some random bits to displaying an image and the answer is: there is a network protocol we must follow in order to handle these messages. Basically, the protocol specifies that we have to read the **data** bit when the **sync** bit switches. Additionally, we have to understand the message format. The data bits are grouped into a message of 17 bits, where the first bit is the control bit and the rest of the 16 bits is the actual data, the payload, the pixel data. The control bit, when it's set on 1, specifies that the transmission of data continues, otherwise the transmission is over. So we receive 17 bit packets up until the control bit, the first bit in the packet, is 0. After the transmission is over, we will have the image printed on the display.

## Solution explanation
I didn't follow the NOTE and managed to implement the solution without using any stack-operation macros. I also defined all the needed macros at the beginning of the program. I will explain their purpose when I'll discuss the code that uses them. I tried to choose suggestive names.
```
...
Now the big loop will start, where the receiving and drawing events are handled, and which will run until the control bit from the message received is 0. 
```
LABEL LOOP
```

As the network protocol specifies, we need to wait for a switch on the **sync** bit. Until then, we cannot proceed towards processing the **data** bit. We first access the network bits, then we apply a bit mask to extract the **sync** bit value and jump to an address depending on the value.
```
...
Once the switch happened, and we exited the wait loop, we store the current **sync** bit for future data readings.
```
...
Now we get to read the **data** bit using the same method for reading the **sync** bit, by applying a mask.
```
...
When we evaluate each payload we use iteration to know which part of the message the **data** bit represents according to the protocol message format. So each message would be of length 17, 1 bit for the control bit and 16 bits for the payload. Based on an index that we store, we check to to see in which part the **data** bit belongs, control bit or payload, and then process it accordingly. We subtract the payload length from the current message index, and if we get a greater than 0 result, it means that the index points to a control bit, otherwise while the result is a negative number, it will point to a pixel bit, which is part of the payload. This index will be decremented later. Each cycle it starts at 17 and decreases at 0, which marks a new message reading next cycle.
```
...
In case the **data** bit is a control bit, we check the **data** bit that we stored to be 0 in order to end the program loop, otherwise we begin processing a new message by jumping at the start of the main loop.
```
...
After that we do a check on the message index after decrementing to see if we are at the end of the payload or not.
```
# decrement index
A = message_index_addr
*A = *A - 1
...
After incrementing the display pixel address, we reset the pixel value and the message index and go back at the beginning of the main loop
```
